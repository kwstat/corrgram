...
- The `seriation` package is moved from Imports to Suggests. This makes
  `corrgram` lighter loading for most people. (Requested by P.Kiener and
  others)
...
- New
  [`panel.fill()`](http://kwstat.github.io/corrgram/reference/corrgram.md)
  function, omits diagonal lines.

- `panel.conf` and `panel.cor` now auto-scale based on absolute value of
  correlations.

- `panel.conf` now only allows `cor.method="pearson"` (which is the
  default.) Fix issue
  [\#13](https://github.com/kwstat/corrgram/issues/13).
...
- [`corrgram()`](http://kwstat.github.io/corrgram/reference/corrgram.md)
  gives a better warning if a symmetric matrix has values outside
  \[-1,1\].
...
- New argument `outer.labels` for adding labels along outside edges of
  corrgram. (Request of Vanessa Bruat and others.)
...
CRAN release: 2015-02-13

- Added more seriate options.

## corrgram 1.6 (2014-08-29)

CRAN release: 2014-08-29

- Moved packages from Depends to Imports.

- Argument `label.pos` now defaults to c(.5, .5) for x,y positioning.
...
CRAN release: 2013-08-29

- Updated references links. (Request of Michael Friendly).

- Fixed small bug with test for correlation matrix. (Reported by F.
  Rosa)
...
CRAN release: 2012-03-28

- Small bug. Now accepts NA values in correlation matrices, and NAs
  caused by missing combinations of data and cor(use=“pair”).
